...
#' @export
render_rst_page <- function(package, name, data, path = "") {
  # render template components
  pieces <- c("header", "content", "footer")
  components <- lapply(pieces, render_rst_template, package = package, name, data)
  names(components) <- pieces

  # render complete layout
  out <- render_rst_template(package, "layout", name, components)
  cat(out, file = path)
}

#' @importFrom whisker whisker.render
render_rst_template <- function(package, type, name, data) {
  template <- readLines(find_rst_template(package, type, name))
...
find_rst_template <- function(package, type, name) {
  paths <- c(
    pkg_sd_path(package),
